...
$.ajaxSetup({
  headers: {
    "X-CSRF-TOKEN": "Y1Oyo3GSdBlyh5958539jvt14rrjU1rsFzrgFVMB",
  },
});
...
//update billing type
var updateBillingType = (value) => {
  document
    .querySelectorAll(".checkout-subscription")
...
if (document.querySelector("#form-payment")) {
  ...
  document
    .querySelectorAll('[name="payment_gateway"]')
    .forEach(function (element) {
      if (element.checked) {
...
document.querySelector("#plan-month") &&
  document.querySelector("#plan-month").addEventListener("click", function () {
    document
      .querySelectorAll(".plan-month")
      .forEach((element) => element.classList.add("d-block"));
    document
      .querySelectorAll(".plan-year")
      .forEach((element) => element.classList.remove("d-block"));
  });

document.querySelector("#plan-year") &&
  document.querySelector("#plan-year").addEventListener("click", function () {
    document
      .querySelectorAll(".plan-year")
      .forEach((element) => element.classList.add("d-block"));
    document
      .querySelectorAll(".plan-month")
      .forEach((element) =>
        element.classList.remove("d-block", "plan-preload")
      );
  });
